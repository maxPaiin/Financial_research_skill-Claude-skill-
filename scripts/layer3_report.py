...
- **Reporting-currency exclusion (v0.32)**: only funds that report AUM in USD enter the days-to-liquidate aggregate. Average daily traded value is always USD, so admitting an AUM reported in HKD or JPY would overstate days-to-liquidate by roughly the exchange rate — silently, with no error and no flag. A fund reporting in another currency, or not stating one, is therefore **excluded from that aggregate** (its holdings still count in full toward overlap, consensus and style diversity), and tickers held only by such funds show **NAV-only** crowding. **No FX conversion is performed anywhere in this pipeline**: converting would require an FX source, a rate-date policy and a new provenance path — three new failure modes to repair a metric that already has a well-defined fallback. An unstated currency is never assumed to be USD.
...
- Consensus counts funds, not exposure: a fund holding only 20–35% of AUM in US equity ("thin US exposure") votes exactly as loudly as a 95%-US fund. Such funds are flagged in Layer 1 and Layer 2 and are deliberately NOT down-weighted — re-weighting would alter the locked composite — so where a material share of the input set is thin, read the consensus signal as weaker still
